...
async function initDB() {
  ...
  db.run(`
    ...
    CREATE TABLE IF NOT EXISTS records (
      ...
      bigBoxPrice REAL DEFAULT 30,
      smallBoxPrice REAL DEFAULT 20,
      createdAt TEXT DEFAULT (datetime('now'))
    );
  ...
  try { db.exec("SELECT bigBox FROM records LIMIT 1"); } catch(e) { db.run("ALTER TABLE records ADD COLUMN bigBox INTEGER DEFAULT 0"); }
  try { db.exec("SELECT smallBox FROM records LIMIT 1"); } catch(e) { db.run("ALTER TABLE records ADD COLUMN smallBox INTEGER DEFAULT 0"); }
  try { db.exec("SELECT bigBoxPrice FROM records LIMIT 1"); } catch(e) { db.run("ALTER TABLE records ADD COLUMN bigBoxPrice REAL DEFAULT 30"); }
  try { db.exec("SELECT smallBoxPrice FROM records LIMIT 1"); } catch(e) { db.run("ALTER TABLE records ADD COLUMN smallBoxPrice REAL DEFAULT 20"); }
  ...
  db.run(`
    ...
    -- 库存核对表：冷库数据（包装员录入）
    CREATE TABLE IF NOT EXISTS stock_cold (
      id INTEGER PRIMARY KEY AUTOINCREMENT,
      date TEXT NOT NULL,
    ...
    -- 库存核对表：货架数据（理货员录入）
    CREATE TABLE IF NOT EXISTS stock_shelf (
      id INTEGER PRIMARY KEY AUTOINCREMENT,
      date TEXT NOT NULL,
      shop INTEGER NOT NULL,
      food TEXT NOT NULL,
      cold_qty REAL DEFAULT 0,
      shelf_action TEXT DEFAULT '', -- 'filled':填了件数, 'no_need':不用采
      shelf_qty REAL DEFAULT 0,
      updatedAt TEXT DEFAULT (datetime('now'))
    );
    -- 报菜数据（从 orders 表按日期汇总到 stock_check_orders，方便库存核对）
  ...
  // 兼容旧表：stock_cold 和 stock_shelf 如果已有旧数据，尝试加 shop 字段
  try { db.exec("SELECT shop FROM stock_cold LIMIT 1"); } catch(e) { db.run("ALTER TABLE stock_cold ADD COLUMN shop INTEGER NOT NULL DEFAULT 1"); }
  try { db.exec("SELECT shop FROM stock_shelf LIMIT 1"); } catch(e) { db.run("ALTER TABLE stock_shelf ADD COLUMN shop INTEGER NOT NULL DEFAULT 1"); }
  try { db.exec("SELECT shop FROM stock_orders LIMIT 1"); } catch(e) { db.run("ALTER TABLE stock_orders ADD COLUMN shop INTEGER NOT NULL DEFAULT 0"); }
  // 兼容 unit 字段
  try { db.exec("SELECT unit FROM stock_cold LIMIT 1"); } catch(e) { db.run("ALTER TABLE stock_cold ADD COLUMN unit TEXT DEFAULT ''"); }
  try { db.exec("SELECT unit FROM stock_shelf LIMIT 1"); } catch(e) { db.run("ALTER TABLE stock_shelf ADD COLUMN unit TEXT DEFAULT ''"); }
...
function addRecord(record) {
  db.run(`
    INSERT INTO records (id, date, time, shop, food, weight, unit, price, money, status, type, bigBox, smallBox, bigBoxPrice, smallBoxPrice)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `, [record.id, record.date, record.time, record.shop, record.food,
       ...
       record.bigBox || 0, record.smallBox || 0, record.bigBoxPrice || 30, record.smallBoxPrice || 20]);
  saveDB();
}
...
function payAllDebt() {
  const stmt = db.prepare("SELECT COUNT(*) as count FROM records WHERE type = '自采' AND status = '未付'");
  stmt.step();
  const count = stmt.getAsObject().count;
  stmt.free();

  db.run("UPDATE records SET status = '已付' WHERE type = '自采' AND status = '未付'");
  saveDB();
  return count;
}
...
function getSummary() {
  ...
  stmt = db.prepare("SELECT COUNT(*) as count, COALESCE(SUM(money), 0) as total FROM records WHERE type = '自采' AND status = '未付'");
  stmt.step();
  const debtStats = stmt.getAsObject();
  stmt.free();

  stmt = db.prepare("SELECT shop, COALESCE(SUM(money), 0) as total FROM records WHERE type = '自采' AND status = '未付' GROUP BY shop ORDER BY total DESC");
  const debtByShop = [];
...
function getAdminStats() {
  ...
  stmt = db.prepare("SELECT COUNT(*) as count FROM records WHERE type = '自采' AND status = '未付'");
  stmt.step();
...
// 统一日期格式：2026-5-19 -> 2026/05/19, 2026/5/19 -> 2026/05/19
function normalizeDate(date) {
  if (!date) return getLocalDateStr(new Date());
  // HTML date input 用 - 分隔，数据库用 /
  const parts = date.split(/[\/-]/);
  if (parts.length === 3) {
    return String(parseInt(parts[0])) + '/' + String(parseInt(parts[1])).padStart(2, '0') + '/' + String(parseInt(parts[2])).padStart(2, '0');
  }
  return date;
}
...
// ====== 库存核对数据访问 ======
const ALL_VEGGIES = [
  "皱皮","泡椒","黄瓜","小瓜","韭菜","山芹","菠菜","茼蒿","苔苔",
  "上海青","奶白菜","空心菜","苦菊","木耳菜","韭菜苔","银甚菜","秋葵",
  "茴香苗","香菜","菜心","小白菜","小香葱","塔菜","黄豆芽","绿豆芽",
  "海带丝","海带结","羊血","鹌鹑蛋","豆泡","豆皮","香豆皮","内脂豆腐",
  "鸡心","鸡杂","豆腐","混沌皮","水饺皮","面条粗","面条细","杂粮面",
  "炒面","炒饼","山药细","山药粗","洋葱","蜜薯","甘蓝","白菜","莴苣",
  "藕","蘑菇","金针菇","海鲜菇","杏鲍菇","香菇","娃娃菜","豆苗","菜花",
  "豆角","米豆","生菜","香椿芽","油豆角","荷兰豆","紫薯","油麦菜",
  "小米辣","彩椒","蒜薹","黄白菜","丝瓜","苦瓜","秋米豆","青花椒",
  "胡萝卜","贝贝","扁青豆","西蓝花","尖椒","茄子","蒜黄","蒜苗","西红柿",
  "土豆","大葱","白萝卜","青萝卜","南瓜","冬瓜","姜","鲜蒜","干蒜",
  "芋头","咸菜疙瘩","粉条","旱黄瓜","鲜毛豆","鲜玉米","鲜花生","紫薯",
  "塔菜","紫花菜","鲜葱","冰激凌萝卜"
];

// 从 orders 表获取某天的报菜数据并同步到 stock_orders
function syncStockOrders(date) {
  // 先清空该日旧数据
  db.run('DELETE FROM stock_orders WHERE date = ?', [date]);

  // 调用方已确保日期归一化，但以防万一
  const normalizedDate = normalizeDate(date);

  const stmt = db.prepare('SELECT * FROM orders WHERE targetDate = ? ORDER BY food');
  stmt.bind([normalizedDate]);
  while (stmt.step()) {
    const row = stmt.getAsObject();
    // 先把报菜存入 shop=0（表示通用）
    db.run('INSERT INTO stock_orders (date, shop, food, qty) VALUES (?, 0, ?, ?)', [date, row.food, parseFloat(row.weight) || 0]);
  }
  stmt.free();
}

// 获取报菜数据
function getStockCheckOrders(date) {
  syncStockOrders(date);

  // 获取通用报菜 (shop=0)
  const stmt = db.prepare('SELECT * FROM stock_orders WHERE date = ? AND shop = 0 ORDER BY food');
  stmt.bind([date]);
  const generalOrders = [];
  while (stmt.step()) generalOrders.push(stmt.getAsObject());
  stmt.free();

  // 同时获取各店的报菜
  const stmt1 = db.prepare('SELECT * FROM stock_orders WHERE date = ? AND shop = 1 ORDER BY food');
  stmt1.bind([date]);
  const shop1Orders = [];
  while (stmt1.step()) shop1Orders.push(stmt1.getAsObject());
  stmt1.free();

  const stmt2 = db.prepare('SELECT * FROM stock_orders WHERE date = ? AND shop = 2 ORDER BY food');
  stmt2.bind([date]);
  const shop2Orders = [];
  while (stmt2.step()) shop2Orders.push(stmt2.getAsObject());
  stmt2.free();

  return { general: generalOrders, shop1: shop1Orders, shop2: shop2Orders, date };
}

// 获取某个店的建议采购汇总（包含冷库、货架、报菜数据）
function getStockCheckSummary(date, shopId) {
  // 同步报菜
  syncStockOrders(date);

  // 构建结果对象
  function getOrdersMap(shop) {
    const stmt = db.prepare('SELECT * FROM stock_orders WHERE date = ? AND shop = ? ORDER BY food');
    stmt.bind([date, shop]);
    const items = [];
    ...
    const map = {};
    items.forEach(i => { map[i.food] = parseFloat(i.qty) || 0; });
    return map;
  }

  function getColdMap(shop) {
    const stmt = db.prepare('SELECT * FROM stock_cold WHERE date = ? AND shop = ? ORDER BY food');
    stmt.bind([date, shop]);
    const items = [];
    while (stmt.step()) items.push(stmt.getAsObject());
    stmt.free();
    ...
    const map = {};
    items.forEach(i => { map[i.food] = { qty: parseFloat(i.qty) || 0, unit: i.unit || '' }; });
    return map;
  }

  function getShelfMap(shop) {
    const stmt = db.prepare('SELECT * FROM stock_shelf WHERE date = ? AND shop = ? ORDER BY food');
    stmt.bind([date, shop]);
    const items = [];
    while (stmt.step()) items.push(stmt.getAsObject());
    stmt.free();
  ...
  const shops = shopId ? [shopId] : [1, 2];
  const results = {};

  shops.forEach(sid => {
    const orders = getOrdersMap(0); // 通用报菜
    const coldMap = getColdMap(sid);
    const shelfItems = getShelfMap(sid);

    // 合并：取 ALL_VEGGIES + 三张表中出现的所有菜品
    const allFoods = new Set(ALL_VEGGIES);
    Object.keys(orders).forEach(f => allFoods.add(f));
    Object.keys(coldMap).forEach(f => allFoods.add(f));
    shelfItems.forEach(i => allFoods.add(i.food));
    // 也加入初始化库存表中出现的菜品
    try {
      const initStmt = db.prepare('SELECT DISTINCT food FROM stock');
      while (initStmt.step()) allFoods.add(initStmt.getAsObject().food);
      initStmt.free();
    } catch {}

    const shopResult = {};
    allFoods.forEach(food => {
      const orderQty = orders[food] || 0;
      const coldData = coldMap[food];
      const coldExists = coldData !== undefined;
      const coldQty = coldExists ? (parseFloat(coldData.qty) || 0) : 0;
      const coldUnit = coldExists ? (coldData.unit || '') : '';

      // 处理货架数据
      const shelfRow = shelfItems.find(i => i.food === food);
      let shelfQty = 0;
      let isEnough = false;
      let isEmpty = false;
      let needJudge = false;
      ...
      if (shelfRow) {
        if (shelfRow.shelf_action === 'no_need') {
          isEnough = true;
        } else if (shelfRow.shelf_action === 'filled') {
          shelfQty = parseFloat(shelfRow.shelf_qty) || 0;
          needJudge = true;
        }
        shelfUnit = shelfRow.unit || '';
      }

      // 计算建议采购量：报菜 - 冷库 - 货架
      const suggest = Math.max(0, orderQty - coldQty - shelfQty);

      if (orderQty > 0 || coldExists || shelfRow) {
        shopResult[food] = { orderQty, coldQty, shelfQty, isEnough, isEmpty, suggest, needJudge, coldExists, coldUnit, shelfUnit };
      }
    });

    results[`shop${sid}`] = shopResult;
  });
...
async function handleAPI(req, res) {
  ...
  try {
    ...
    // GET /api/price/latest/:shop - 查询某商家最新框子单价
    const priceMatch = pathname.match(/^\/api\/price\/latest\/(.+)$/);
    if (priceMatch && method === 'GET') {
      const shop = decodeURIComponent(priceMatch[1]);
      const stmt = db.prepare('SELECT bigBoxPrice, smallBoxPrice FROM records WHERE shop = ? AND (bigBox > 0 OR smallBox > 0) ORDER BY createdAt DESC LIMIT 1');
      stmt.bind([shop]);
      let result = { bigBoxPrice: 30, smallBoxPrice: 20 };
    ...
    if (pathname === '/api/records/pay-shop' && method === 'PUT') {
      ...
      const stmt = db.prepare("SELECT COUNT(*) as c FROM records WHERE shop = ? AND type = '自采' AND status = '未付'");
      stmt.bind([shop]);
      stmt.step();
      const count = stmt.getAsObject().c;
      stmt.free();
      db.run("UPDATE records SET status = '已付' WHERE shop = ? AND type = '自采' AND status = '未付'", [shop]);
      saveDB();
      return sendJSON(res, 200, { success: true, count });
    }
    ...
    if (pathname === '/api/stock/overview' && method === 'GET') {
      const date = normalizeDate(url.searchParams.get('date') || getLocalDateStr(new Date()));
    ...
    // ====== 库存核对 API ======
    // GET /api/stock-check/orders?date=X - 获取某日报菜数据（从 orders 表汇总）
    if (pathname === '/api/stock-check/orders' && method === 'GET') {
      const date = normalizeDate(url.searchParams.get('date') || getLocalDateStr(new Date()));
      const result = getStockCheckOrders(date);
      return sendJSON(res, 200, result);
    }

    // POST /api/stock-check/sync-orders - 同步报菜数据到 stock_orders 表
    if (pathname === '/api/stock-check/sync-orders' && method === 'POST') {
      const body = await parseBody(req);
      const date = normalizeDate(body.date || getLocalDateStr(new Date()));
      syncStockOrders(date);
      saveDB();
      return sendJSON(res, 200, { success: true, date });
    }

    // GET /api/stock-check/cold?date=X&shop=X - 获取冷库数据（包装员）
    if (pathname === '/api/stock-check/cold' && method === 'GET') {
      const date = normalizeDate(url.searchParams.get('date') || getLocalDateStr(new Date()));
      const shop = parseInt(url.searchParams.get('shop')) || 1;
      const stmt = db.prepare('SELECT * FROM stock_cold WHERE date = ? AND shop = ? ORDER BY food');
      stmt.bind([date, shop]);
      const items = [];
      while (stmt.step()) items.push(stmt.getAsObject());
      stmt.free();
      return sendJSON(res, 200, { items, date, shop });
    }

    // POST /api/stock-check/cold - 保存冷库数据（包装员提交）
    if (pathname === '/api/stock-check/cold' && method === 'POST') {
      const body = await parseBody(req);
      const { date, shop, items } = body;
      if (!date || !shop || !Array.isArray(items)) return sendJSON(res, 400, { error: '缺少必填字段' });

      // 先清空该店该日期的旧数据
      db.run('DELETE FROM stock_cold WHERE date = ? AND shop = ?', [date, shop]);

      // 批量插入
      const stmt = db.prepare('INSERT INTO stock_cold (date, shop, food, qty, unit) VALUES (?, ?, ?, ?, ?)');
      items.forEach(item => {
        stmt.bind([date, shop, item.food, parseFloat(item.qty) || 0, item.unit || '']);
        stmt.step();
        stmt.reset();
      });
      stmt.free();
      saveDB();
      return sendJSON(res, 200, { success: true, count: items.length });
    }

    // GET /api/stock-check/shelf?date=X&shop=X - 获取货架核对数据（理货员）
    if (pathname === '/api/stock-check/shelf' && method === 'GET') {
      const date = normalizeDate(url.searchParams.get('date') || getLocalDateStr(new Date()));
      const shop = parseInt(url.searchParams.get('shop')) || 1;
      const stmt = db.prepare('SELECT * FROM stock_shelf WHERE date = ? AND shop = ? ORDER BY food');
      stmt.bind([date, shop]);
      ...
      // 同时返回对应的冷库数据
      const coldStmt = db.prepare('SELECT * FROM stock_cold WHERE date = ? AND shop = ? ORDER BY food');
      coldStmt.bind([date, shop]);
      const coldItems = [];
      while (coldStmt.step()) coldItems.push(coldStmt.getAsObject());
      coldStmt.free();

      return sendJSON(res, 200, { items, coldItems, date, shop });
    }

    // POST /api/stock-check/shelf - 保存货架核对数据（理货员提交）
    if (pathname === '/api/stock-check/shelf' && method === 'POST') {
      const body = await parseBody(req);
      const { date, shop, items } = body;
      if (!date || !shop || !Array.isArray(items)) return sendJSON(res, 400, { error: '缺少必填字段' });

      // 先清空该店该日期的旧数据
      db.run('DELETE FROM stock_shelf WHERE date = ? AND shop = ?', [date, shop]);

      // 批量插入
      const stmt = db.prepare('INSERT INTO stock_shelf (date, shop, food, cold_qty, shelf_action, shelf_qty, unit) VALUES (?, ?, ?, ?, ?, ?, ?)');
      items.forEach(item => {
        stmt.bind([date, shop, item.food, parseFloat(item.cold_qty) || 0, item.shelf_action || '', parseFloat(item.shelf_qty) || 0, item.unit || '']);
        stmt.step();
        stmt.reset();
      });
      stmt.free();
    ...
    // GET /api/stock-check/summary?date=X&shop=X - 获取单个店的建议采购总览
    if (pathname === '/api/stock-check/summary' && method === 'GET') {
      const date = normalizeDate(url.searchParams.get('date') || getLocalDateStr(new Date()));
      const shop = url.searchParams.get('shop');
      return sendJSON(res, 200, getStockCheckSummary(date, shop ? parseInt(shop) : null));
    }

    // GET /api/stock-check/actual-purchase?date=X - 获取采购A当天自采记录（用于分货参考）
    if (pathname === '/api/stock-check/actual-purchase' && method === 'GET') {
      const date = normalizeDate(url.searchParams.get('date') || getLocalDateStr(new Date()));
      const stmt = db.prepare('SELECT food, SUM(weight) as totalWeight FROM records WHERE date = ? AND type = ? GROUP BY food ORDER BY food');
      stmt.bind([date, '自采']);
      const items = [];
      while (stmt.step()) items.push(stmt.getAsObject());
      stmt.free();
      return sendJSON(res, 200, { items, date });
    }
    ...
    if (pathname === '/api/orders' && method === 'GET') {
      ...
      // 统一日期格式
      const targetDate = normalizeDate(targetDateParam);
    ...
    if (pathname === '/api/backup' && method === 'POST') {
      try {
        const today = getLocalDateStr(new Date()).replace(/\//g, '-');
        const backupPath = path.join(DATA_DIR, 'backup-' + today + '.db');
        fs.writeFileSync(backupPath, Buffer.from(db.export()));
        return sendJSON(res, 200, { success: true, path: backupPath, date: today });
      } catch(e) {
        return sendJSON(res, 500, { error: e.message });
      }
    }

    // API: 所有出现过菜品（包装员/理货员新加的商品也能被所有人看见）
    if (pathname === '/api/foods' && method === 'GET') {
      try {
        ...
        const onlyPreset = url.searchParams.get('preset') === 'true';
        if (onlyPreset) {
          return sendJSON(res, 200, { foods: preset, extra: [] });
        }
        // 收集新增菜品（不在预制列表中的）
        const extra = new Set();
        ['stock_cold', 'stock_shelf', 'stock_orders', 'stock', 'orders', 'records'].forEach(t => {
          try {
            const r = db.exec("SELECT DISTINCT food FROM `" + t + "` WHERE food IS NOT NULL AND food != ''");
            if (r.length > 0) r[0].values.forEach(row => { if (!preset.includes(row[0])) extra.add(row[0]); });
          } catch {}
        });
        return sendJSON(res, 200, { foods: preset.concat(Array.from(extra).sort()), extra: Array.from(extra) });
      } catch(e) {
        return sendJSON(res, 500, { error: e.message });
      }
    }

    // API: 当日已采购菜品（给采购查看页面标记已购）
    if (pathname === '/api/purchased-today' && method === 'GET') {
      const date = normalizeDate(url.searchParams.get('date') || getLocalDateStr(new Date()));
      try {
        const r = db.exec(
          "SELECT DISTINCT shop, food FROM records WHERE date = ? AND type = '自采'",
          [date]
        );
        const items = [];
        if (r.length > 0) {
          r[0].values.forEach(row => items.push({ shop: parseInt(row[0]), food: row[1] }));
        }
        return sendJSON(res, 200, { items, date });
      } catch(e) {
        return sendJSON(res, 500, { error: e.message });
    ...
    if (pathname === '/api/stats' && method === 'GET') {
      try {
        const tables = ['shops','records','orders','deposits','sales','stock_cold','stock_shelf','stock_orders'];
        const stats = {};
        tables.forEach(t => {
          try {
            const r = db.exec("SELECT COUNT(*) as cnt FROM `" + t + "`");
            stats[t] = r.length > 0 ? r[0].values[0][0] : 0;
          } catch { stats[t] = 0; }
        });
        // stock 表的别名查询
        try {
          const r = db.exec("SELECT COUNT(*) as cnt FROM stock");
          stats['stock'] = r.length > 0 ? r[0].values[0][0] : 0;
        } catch { stats['stock'] = 0; }
